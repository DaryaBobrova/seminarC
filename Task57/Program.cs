...
//int[,] frequenceDictionary = FrequencyDictionary(arr);
//PrintFrequencyDictionary(frequenceDictionary);
...
void PrintFrequencyArray(int[,] frequencyArray) // вывод частотного массива на консоль
{
    for (int i = 0; i < frequencyArray.GetLength(0); i++)
    {
        ...
        Console.WriteLine($"{frequencyArray[i, 0]} встречается {frequencyArray[i, 1]} раз(а)");
    }
}
